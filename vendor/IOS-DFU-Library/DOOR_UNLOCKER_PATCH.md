...
- The patched client uses `CBPeripheral.maximumWriteValueLength(for: .withoutResponse)` for the known factory `AdaDFU` and reproducible `DoorDFU` bootloaders, with a 244-byte cap. Unknown Legacy DFU bootloaders keep 20-byte packets.
