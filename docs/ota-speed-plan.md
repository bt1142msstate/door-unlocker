...
- Exact `v0.3.0-beta.1` Mac proof exposed a client regression: `DoorDFUStable` missed an exact `DoorDFU` packet-size gate and fell back to 20-byte writes, producing roughly `1.2 KB/s` (`111s` upload). The gate now recognizes the project-owned `DoorDFU*` family in both apps.
- Post-fix wireless-only iPhone proof: PRN `9` selected a `244`-byte payload and uploaded the `135,495`-byte signed package in `6.12s`, with roughly `22-25 KB/s` average progress throughput and post-reboot BLE verification in `22s` end to end.
- Post-fix Mac proof: the same shared repair reduced signed upload time from `111s` to `40s`. This confirms the 20-byte fallback was the primary regression; macOS CoreBluetooth scheduling remains slower than the iPhone on the same controller.
